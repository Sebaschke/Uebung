function intApprox = mittelpunktSumme( f, a, b, n )
...
% mit Hilfe der zusammengesetzen Mittelpunktsregel
%
...
% Letzte Aenderung: 13.07.2020
  ...
  xvals     = linspace( a + h/2, b - h/2, n );
  
  % Gewichte (Spaltenvektor)
  weights   = ones( n, 1 );
  
  % Berechnung des Integrals (Skalarprodukt)
  intApprox = h * f( xvals ) * weights;
  
  % Alternative Berechnung des Integrals
  % intApprox = h * sum( f( xvals ) );
end